---
name: auto
description: Autonomous build loop with Karpathy ratcheting, GAN evaluator, and session chaining. Iterates story groups until all features pass or stopping criteria met. Use when asked to build autonomously, run the full pipeline, or iterate until all features pass.
allowed-tools: shell
---
...
- `/auto` is the orchestrator. It reads state, makes decisions, invokes custom agents, and manages the loop.
- Code generation is delegated to the **generator** custom agent (via `/implement` or direct agent invocation).
- Code verification is delegated to the **evaluator** custom agent (via `/evaluate` or direct agent invocation).
- Design critique is delegated to the **design-critic** custom agent.
...
Sprint contracts define the verifiable done-criteria for a group. Two-step propose-approve process using generator and evaluator custom agents.

### Step 2 — Generator Proposes Contract

Invoke the generator custom agent with this prompt:
...
Invoke the evaluator custom agent with this prompt:
...
Invoke the generator custom agent to create and manage a team of custom agents for the current group.
...
Note: Agent teams are invoked via custom agents. Each teammate is a custom agent invocation with its own prompt and file ownership scope.
...
Invoke the evaluator custom agent to verify `architecture_checks` from the sprint contract:
...
Invoke the evaluator custom agent with the full sprint contract. The evaluator runs:
...
Invoke the design-critic custom agent on every page listed in the sprint contract's `design_checks`. The critic screenshots each page, scores visual fidelity, and returns PASS/FAIL per check. See SECTION 9 for the full GAN loop if scores are below threshold.
...
3. **Invoke the generator custom agent** to apply the targeted fix. The generator prompt must include:
...
2. If new amendment files are found:
   ...
   - Invoke the planner custom agent to update affected architecture artifacts (`api-contracts.md`, `component-map.md`, schema files).
...
2. **Score** — Invoke the design-critic custom agent with screenshots + calibration profile
...
- **Ignoring failure log patterns:** Check `failures.md` for recurring patterns BEFORE invoking the generator custom agent. If the same error has appeared before, inject the relevant learned rule into the generator prompt proactively.
...
- **Not injecting learned rules:** Every agent prompt must include the full text of all learned rules. This is the most common cause of repeated failures. If you invoke a custom agent without learned rules, you are guaranteeing a preventable regression.
